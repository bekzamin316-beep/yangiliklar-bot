...
def _render(events: list[dict], *, now: datetime | None = None) -> bytes:
    ...
    Bars show the percentage of total supply being unlocked; labels carry
    the token symbol + unlock date, and each bar is annotated with the USD
    value plus the remaining locked supply.
    """
    ...
    for i, ev in enumerate(top):
        ...
        pct = float(nu.get("tokenAmountPercentage") or 0.0)
        locked = float(ev.get("tokenLockedAmount") or 0.0)
        symbol = str(ev.get("symbol", "?"))
        when = ev["_when"]
        date_str = f"{when.day} {_UZ_MONTHS[when.month - 1]}"
        labels.append(f"{symbol}  ·  {date_str}")
        values.append(pct if pct > 0 else 0.0)
        colors.append(_RANK_COLORS[i % len(_RANK_COLORS)])
        note = f"{_fmt_usd(usd)}  ·  {str(pct).replace('.', ',')}% unlock"
        if locked > 0:
            note += f"\nqoldi: {_fmt_tokens(locked)} token"
        annotations.append(note)

    fig, ax = plt.subplots(figsize=(12, max(6.5, 0.78 * len(top) + 2.4)))
    ...
    for bar, note in zip(bars, annotations):
        ...
        ax.text(
            x, bar.get_y() + bar.get_height() / 2, note,
            va="center", ha="left", fontsize=9.5, color=_TEXT,
            linespacing=1.4,
        )
    ...
    ax.text(
        0.0, -0.16,
        "Bar: taqsimotdan ochilayotgan ulush (%)  ·  Manba: CoinMarketCap",
        transform=ax.transAxes, fontsize=9, color=_MUTED,
    )
    ax.set_xlim(0, max_val * 1.45 if max_val else 1.0)
